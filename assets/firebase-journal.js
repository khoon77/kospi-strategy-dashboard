...
import { getAuth, GoogleAuthProvider, onAuthStateChanged, signInWithPopup, signInWithRedirect, signOut } from 'https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js';
import { collection, deleteDoc, doc, getDocs, getFirestore, onSnapshot, setDoc } from 'https://www.gstatic.com/firebasejs/12.16.0/firebase-firestore.js';

const firebaseConfig = {
  apiKey: 'AIzaSyDc29tV9ipg7fGMh3Nngjvar8WMS2npHdM', authDomain: 'kospi-strategy.firebaseapp.com',
  projectId: 'kospi-strategy', storageBucket: 'kospi-strategy.firebasestorage.app',
  messagingSenderId: '647193420942', appId: '1:647193420942:web:6934229fc78615401515df', measurementId: 'G-KS5E6W7F41'
};
const app = initializeApp(firebaseConfig), auth = getAuth(app), db = getFirestore(app), provider = new GoogleAuthProvider();
const LOCAL_DB = 'kospiJournal', LOCAL_STORE = 'entries';
let user = null, stopWatching = null, syncing = false;
const cloudStyle = document.createElement('link'); cloudStyle.rel = 'stylesheet'; cloudStyle.href = 'assets/firebase-journal.css?v=20260802-1'; document.head.appendChild(cloudStyle);

function openLocalDB() { return new Promise((resolve, reject) => { const request = indexedDB.open(LOCAL_DB, 1); request.onupgradeneeded = () => { if (!request.result.objectStoreNames.contains(LOCAL_STORE)) request.result.createObjectStore(LOCAL_STORE, { keyPath: 'id' }); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); }
async function localEntries() { const localDB = await openLocalDB(); return new Promise((resolve, reject) => { const request = localDB.transaction(LOCAL_STORE).objectStore(LOCAL_STORE).getAll(); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); }
async function putLocal(entry) { const localDB = await openLocalDB(); return new Promise((resolve, reject) => { const request = localDB.transaction(LOCAL_STORE, 'readwrite').objectStore(LOCAL_STORE).put(entry); request.onsuccess = () => resolve(); request.onerror = () => reject(request.error); }); }
function status(message, tone = '') { const el = document.getElementById('cloud-status'); if (el) { el.textContent = message; el.dataset.tone = tone; } }
function refreshJournal() { const tab = document.querySelector('[data-view="journal"]'); if (tab && !document.getElementById('journal-view')?.hidden) tab.click(); }

function compress(dataUrl, name) { return new Promise((resolve, reject) => { const image = new Image(); image.onerror = () => reject(new Error(`${name || '이미지'} 압축에 실패했습니다.`)); image.onload = () => { let maxSide = 900, quality = .68, result = dataUrl; for (let attempt = 0; attempt < 6; attempt += 1) { const scale = Math.min(1, maxSide / Math.max(image.naturalWidth, image.naturalHeight)), canvas = document.createElement('canvas'); canvas.width = Math.max(1, Math.round(image.naturalWidth * scale)); canvas.height = Math.max(1, Math.round(image.naturalHeight * scale)); canvas.getContext('2d').drawImage(image, 0, 0, canvas.width, canvas.height); result = canvas.toDataURL('image/jpeg', quality); if (result.length < 720000) break; maxSide = Math.round(maxSide * .82); quality = Math.max(.45, quality - .05); } result.length >= 900000 ? reject(new Error(`${name || '이미지'} 용량이 너무 큽니다.`)) : resolve(result); }; image.src = dataUrl; }); }
function journalRef(id) { return doc(db, 'users', user.uid, 'journals', id); }

async function uploadEntry(entry) {
  if (!user || !entry?.id) return;
  const plain = { ...entry }; delete plain.strategyImages; delete plain.tradeImages; plain.ownerUid = user.uid; plain.syncedAt = new Date().toISOString();
  await setDoc(journalRef(entry.id), plain);
  const photosRef = collection(journalRef(entry.id), 'photos'), existing = await getDocs(photosRef), keep = new Set();
  for (const [kind, photos] of [['strategy', entry.strategyImages || []], ['trade', entry.tradeImages || []]]) for (let index = 0; index < photos.length; index += 1) { const photo = photos[index], photoId = `${kind}-${index}`; keep.add(photoId); await setDoc(doc(photosRef, photoId), { kind, index, name: photo.name || `${kind}-${index}.jpg`, type: 'image/jpeg', data: await compress(photo.data, photo.name), updatedAt: plain.updatedAt || plain.syncedAt }); }
  await Promise.all(existing.docs.filter(item => !keep.has(item.id)).map(item => deleteDoc(item.ref)));
}
async function remoteEntry(snapshot) { const entry = { id: snapshot.id, ...snapshot.data(), strategyImages: [], tradeImages: [] }; delete entry.ownerUid; delete entry.syncedAt; const photos = await getDocs(collection(snapshot.ref, 'photos')); photos.docs.map(item => item.data()).sort((a, b) => a.index - b.index).forEach(photo => (photo.kind === 'trade' ? entry.tradeImages : entry.strategyImages).push({ name: photo.name, type: photo.type, data: photo.data })); return entry; }
