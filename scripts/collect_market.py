...
ROOT=Path(__file__).resolve().parents[1]; FILE=ROOT/'data'/'market.json'; SEOUL=timezone(timedelta(hours=9)); DEFAULT='https://data-dbg.krx.co.kr/svc/apis/idx/kospi_dd_trd'
def n(v):
...
def main():
 ...
 try:
  ...
  rows=payload.get('OutBlock_1') or payload.get('output') or []; row=next((x for x in rows if str(x.get('IDX_NM','')).strip().upper()=='KOSPI'),None)
  if not row: print(f'No KOSPI row for {date}; holiday or data pending.');return 0
  rec={'date':date,'open':n(row.get('OPNPRC_IDX')),'high':n(row.get('HGPRC_IDX')),'low':n(row.get('LWPRC_IDX')),'close':n(row.get('CLSPRC_IDX')),'change_pct':n(row.get('FLUC_RT')),'trade_value':n(row.get('ACC_TRDVAL')),'foreign_spot_net':None,'foreign_futures_net':None,'source':'KRX Open API'}
  ...
  data.update(status='live',message='KRX 지수 수집 완료',updated_at=now.isoformat(timespec='seconds'),records=records[-750:]);FILE.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(f"Collected {date}: {rec['close']}");return 0
